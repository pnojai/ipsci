...
Here we try to increase the topics to 200 (roughly 1/8 of the documents). 
```{r}
gc(abstract_lda)
...
abstract_one_term_topic <- top_terms[seq(1,nrow(top_terms),10),]
...
Merge
```{r}
doc_topic_terms <- abstract_one_term_topic %>% transmute(docId=docId, topic=topic, term=term) 
patents_topic_terms <- merge(doc_topic_terms, patents, by='docId')

kable(head(patents_topic_terms, n=2))
```
