...
introduction to searching for research papers for which we will utilize the R package, `rplos`, a programmatic interface to the Solr based search API
...
Executed, variables serialized, chunk disabled.
```{r, eval=FALSE}
...
```{r, eval=FALSE}
We observe that each patent includes citations to reference other patents, as well as those other patents that cite it. We then ask if it might be possible to identify counts of each to demonstrate, for instance:
* a rough proxy metric of the referenciblity of each patent (how much more or less is a given patent cited?)
* the count of citations over time

This analysis will help us begin to answer the question, "how, and to what extent, do patents build on other patents?" Subsequent analysis should account for patent density across time.
```{r}
# cited by (count) 
...
Chunk disabled. Execute by hand if refreshing the CSV is necessary.

```{r, eval=FALSE}
write.csv(raw.df,"data/patent_text.csv", row.names = FALSE)
saveRDS(count.citations, file.path("data", "count.citations.RDS"))
saveRDS(count.cited_by, file.path("data", "raw.html.RDS"))
saveRDS(raw.html, file.path("data", "count.cited_by.RDS"))
saveRDS(raw.df, file.path("data", "raw.df.RDS"))
saveRDS(app_num, file.path("data", "app_num.RDS"))
saveRDS(citations, file.path("data", "citations.RDS"))
saveRDS(cited_by, file.path("data", "cited_by.RDS"))
saveRDS(raw.abstract, file.path("data", "raw.abstract.RDS"))
saveRDS(raw.background, file.path("data", "raw.background.RDS"))
saveRDS(raw.origin, file.path("data", "raw.origin.RDS"))
saveRDS(raw.title, file.path("data", "raw.title.RDS"))
saveRDS(title, file.path("data", "title.RDS"))
```
...
patents <- patents %>%
  ...
  mutate(title=str_replace_all(title,'apparatus|article|control|device|include.*|provide.*|methods*|systems*','')) %>%
  ...
  mutate(abstract=str_replace_all(abstract,'apparatus|article|control|device|include.*|provide.*|methods*|systems*','')) %>%
...
```{r}
terms_ <- subset(abs_topics_term, topic==topics_$docId, -beta)  

topic_terms <- inner_join(topics_, terms_, by = "topic")
```
...
## Findings
NASA patents appear to have peaked in count between 2005 and 2008.  
