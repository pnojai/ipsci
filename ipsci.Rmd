...
### extract patent data from Google HTML
```{r}
...
### 
```{r}
#raw.html <- read_html("data/5827531.html")
# title
#raw.html %>% html_nodes("body search-app article h1") %>% html_text() %>% .[[1]]
# abstract
#raw.html %>% html_nodes("abstract") %>% html_text()
...
lapply(files, function(x) {
  ...
  # apply function
  a <- raw.html %>% html_nodes("body search-app article h1") %>% html_text() %>% .[[1]] %>% str_remove(" \n        - Google Patents")
  b <- raw.html %>% html_nodes("abstract") %>% html_text()
})
```
