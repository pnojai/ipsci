...
raw.html <- read_html("data/6918970.html")
# title
...
#todo: the Google patent retrieval function above returns 2 files with "404" in the title, where the filename includes a comma. We need to either update the retrieval function OR update the function below to ignore those files. For now, the below function works when you manually remove the two "bad" files, but please note that this won't run smoothly without this workaround otherwise. Nich will look into this. 

### clean and transform selected data into dataframe
```{r}
# create object with all filenames
files <- list.files(path="data", pattern="*.html", full.names=TRUE, recursive=FALSE)
...
count.cited_by <- lapply(cited_by, function(x) {
  # apply function
  count.cited_by <- nrow(x[[1]])
  print(count.cited_by)
})
count.cited_by
...
raw.df <- setNames(do.call(rbind.data.frame, Map('c', app_num, title, raw.abstract, raw.origin, raw.background)), c("appId", "title", "abstract", "origin", "background"))
raw.df
```
